...
/* ===================================================== */
/* 🧪 TEMPORARY GOOGLE PLAY API CONNECTION TEST          */
...
app.get(
  ...
  async (req, res) => {
    try {
      const suppliedSecret = String(
        req.headers["x-tunevora-admin-secret"] || ""
      ).trim();
      ...
      const [standardResponse, premiumResponse] =
        await Promise.all([
          publisher.monetization.subscriptions.get({
            packageName:
              GOOGLE_PLAY_PACKAGE_NAME,
            productId:
              GOOGLE_PLAY_STANDARD_PRODUCT_ID,
          }),
      ...
      console.log(
        "✅ GOOGLE PLAY DEVELOPER API CONNECTION OK"
      );

      return res.json({
        success: true,
        message:
          "Google Play Developer API connection is working",
        package_name:
          GOOGLE_PLAY_PACKAGE_NAME,
        products: [
          standardResponse.data.productId ||
            GOOGLE_PLAY_STANDARD_PRODUCT_ID,
          premiumResponse.data.productId ||
            GOOGLE_PLAY_PREMIUM_PRODUCT_ID,
        ],
      });
    } catch (error) {
      console.log(
        "❌ GOOGLE PLAY API TEST ERROR:",
        error.response?.data ||
          error.message
      );
