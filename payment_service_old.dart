...
class PaymentService {
  ...
  static const String googlePlayStandardProductId = 'standard_monthly';
  static const String googlePlayPremiumProductId = 'premium_monthly';

  static const Map<String, String> _googlePlayProductByPlan =
      <String, String>{
    'standard': googlePlayStandardProductId,
    'premium': googlePlayPremiumProductId,
  };
  ...
  /// Purchases a Tunevora subscription through Google Play Billing.
  static Future<PaymentResult> payWithGooglePlay({
    required String plan,
  }) async {
    final normalizedPlan = _normalizeGooglePlayPlan(plan);
    final user = _requireUser();

    if (kIsWeb || defaultTargetPlatform != TargetPlatform.android) {
      throw const PaymentServiceException(
        'Google Play Billing is only available in the Android app.',
      );
    }
    ...
    final productId = _googlePlayProductByPlan[normalizedPlan]!;
    ...
    if (productResponse.notFoundIDs.contains(productId) ||
      ...
      throw PaymentServiceException(
        'Google Play could not find the $productId subscription. '
        'Install Tunevora from Google Play Internal Testing and try again.',
      );
    }
    ...
    purchaseSubscription = _inAppPurchase.purchaseStream.listen(
      (purchases) async {
        for (final purchase in purchases) {
          ...
          try {
            ...
            if (purchase.status == PurchaseStatus.canceled) {
              completer.complete(
                PaymentResult.cancelled(
                  ...
                  message: 'Google Play purchase was cancelled.',
                ),
              );
              continue;
            }
            ...
            if (purchase.status == PurchaseStatus.purchased ||
              ...
              final verification = await _verifyGooglePlayPurchase(
                productId: productId,
                purchaseToken:
              ...
              if (purchase.pendingCompletePurchase) {
                try {
                  ...
                  debugPrint(
                    'Google Play local completion warning: $error',
                  );
                }
              }
              ...
              completer.complete(
                PaymentResult.success(
                  ...
                  currency: product.currencyCode,
                  reference:
                      purchase.purchaseID ?? verification.productId,
                  message:
                      '${_planTitle(verification.plan)} activated through Google Play.',
                ),
              );
            }
        ...
        if (!completer.isCompleted) {
          completer.completeError(
            PaymentServiceException(
              'Google Play purchase updates could not be received.',
              cause: error,
            ),
            stackTrace,
          );
    ...
    try {
      ...
      if (!purchaseStarted) {
        throw const PaymentServiceException(
          'Google Play did not start the purchase.',
        );
      }
      ...
      return await completer.future.timeout(
        ...
        onTimeout: () {
          throw const PaymentServiceException(
            'Google Play purchase is still pending. Check the Play Store '
            'payment status and use Restore purchases if needed.',
          );
        },
      );
      ...
      throw PaymentServiceException(
        'Google Play purchase could not be completed.',
        cause: error,
      );
    } finally {
  ...
  /// Restores and verifies an existing Google Play subscription.
  static Future<PaymentResult> restoreGooglePlayPurchases() async {
    _requireUser();

    if (kIsWeb || defaultTargetPlatform != TargetPlatform.android) {
      throw const PaymentServiceException(
        'Google Play restore is only available in the Android app.',
      );
    }
    ...
    purchaseSubscription = _inAppPurchase.purchaseStream.listen(
      (purchases) async {
        for (final purchase in purchases) {
          final plan = _googlePlayPlanForProduct(purchase.productID);
          ...
          try {
            ...
            if (purchase.pendingCompletePurchase) {
              try {
                ...
                debugPrint(
                  'Google Play restore completion warning: $error',
                );
              }
            }
            ...
            completer.complete(
              PaymentResult.success(
                ...
                reference: purchase.purchaseID,
                message:
                    '${_planTitle(verification.plan)} restored through Google Play.',
              ),
            );
          } catch (error, stackTrace) {
            if (!completer.isCompleted) {
      ...
      onError: (Object error, StackTrace stackTrace) {
        if (!completer.isCompleted) {
          completer.completeError(
            PaymentServiceException(
              'Google Play purchases could not be restored.',
              cause: error,
            ),
            stackTrace,
          );
      ...
      return await completer.future.timeout(
        ...
        onTimeout: () {
          return PaymentResult.processing(
            ...
            plan: 'free',
            message:
                'No active Tunevora Google Play subscription was found.',
          );
        },
      );
    } finally {
  ...
  static Future<_GooglePlayVerificationData> _verifyGooglePlayPurchase({
    ...
    if (session == null || session.accessToken.trim().isEmpty) {
      throw const PaymentServiceException(
        'Your Tunevora session expired. Please sign in again.',
      );
    }
    ...
    final verifiedPlan = _readNullableString(response['plan']);
    final verifiedProductId =
        _readNullableString(response['product_id']) ?? productId;

    if (!success || verifiedPlan == null) {
      throw const PaymentServiceException(
        'Tunevora could not verify the Google Play subscription.',
      );
    }

    return _GooglePlayVerificationData(
      plan: verifiedPlan,
      productId: verifiedProductId,
      premiumUntil: DateTime.tryParse(
        _readNullableString(response['premium_until']) ?? '',
      ),
    );
  }
  ...
  static String _normalizeGooglePlayPlan(String plan) {
    final normalized = plan.trim().toLowerCase();

    if (!_googlePlayProductByPlan.containsKey(normalized)) {
      throw const PaymentServiceException(
        'Google Play supports only the Standard and Premium plans.',
      );
    }
  ...
  static String? _googlePlayPlanForProduct(String productId) {
    for (final entry in _googlePlayProductByPlan.entries) {
...
class _GooglePlayVerificationData {
  const _GooglePlayVerificationData({
    required this.plan,
